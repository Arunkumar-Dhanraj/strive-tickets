...
export default function CategoryKeywords({ categoryKeywords }) {
    ...
    const getTicket = async () => {
        ...
        try {
            ...
            if (data.ticket) {
                const ticketText = `${data.ticket.subject} ${data.ticket.description}`.toLowerCase();
                let matchedCategories = [];

                categoryKeywords.forEach(({ category, keywords }) => {
                    const keywordList = keywords.split(","); 
                    if (keywordList.some(keyword => ticketText.includes(keyword.trim().toLowerCase()))) {
    ...
    return (
        <>
            ...
            <div className="flex">
                <div className="pt-2">
                    <h1 className="pt-2 pb-3">Ticket ID</h1>
                    <input 
